...
locals {
  ...
  // enable_cloudwatch_loggingがtrueの場合、デフォルトのlogDriverをawslogsに設定
  // 他のlogDriverを使用する場合は、log_configurationの変数を設定する
  // また、awslogs-region, awslogs-group, awslogs-stream-prefixを設定
  log_configuration = merge(
    { for k, v in {
      logDriver = "awslogs",
      options = {
        awslogs-region        = data.aws_region.current.name,
        awslogs-group         = try(aws_cloudwatch_log_group.this[0].name, ""), // 保存先のCloudWatch log group名
  ...
  definition = {
    ...
    logConfiguration       = length(local.log_configuration) > 0 ? local.log_configuration : null // log_configurationが設定されている場合は設定される
    memory                 = var.memory
...
resource "aws_cloudwatch_log_group" "this" {
  ...
  // name_prefixがtrueの場合はnameをnullにする
  name              = var.cloudwatch_log_group_use_name_prefix ? null : local.log_group_name       # name_prefixがtrueの場合はnameをnullにする
  name_prefix       = var.cloudwatch_log_group_use_name_prefix ? "${local.log_group_name}-" : null # name_prefixがtrueの場合はname_prefixを設定
  retention_in_days = var.cloudwatch_log_group_retention_in_days                                   // logの保持期間
  kms_key_id        = var.cloudwatch_log_group_kms_key_id                                          // 暗号化に使用するKMSキー
